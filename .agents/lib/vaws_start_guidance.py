...
def guidance(client: str | None = None) -> str:
    ...
    return (
        ...
        "For first-use setup or independent editing without prepared task facts, reuse a valid completed "
        "`.vaws-local/onboarding.json` from this workspace or its explicitly recorded owner. "
        "Otherwise read `.agents/bootstrap/repo-init/SKILL.md` and offer only missing setup choices. "
        "A saved `.vaws-local/github.json` confirms a username, not Fork, Star or community choices. "
        "Reuse explicit answers and saved progress; a new task is not a new user. "
        "Existing setup needs no authentication or network probe.\n\n"
        ...
        "New tasks select the local fixed commit and a matching ready cache; `--latest` explicitly checks upstream. "
        "The default operation directory is the selected Ascend repository; use `--repo workspace` or "
        "`--repo vllm` for an explicit alternative.\n\n"
        "Keep the returned `workspace` as W, the root for task configuration, Skills and environment. "
        "Use returned `cwd` and `repository` for business shell/Git operations and their commit target, "
        "even if the client UI shows the original project. Use absolute paths under W for VAWS "
        "scripts and Skills; a business cwd does not contain W's `.agents` directory. Sources and "
