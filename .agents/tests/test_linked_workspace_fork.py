...
from vaws_native_workspace import create_workspace, git
...
def test_independent_copy_preserves_stage_working_untracked_and_component_edits(tmp_path):
    ...
    (source / "ignored.cache").write_text("not editing state", encoding="utf-8")
    module = source / "vllm"
    ...
    assert (target / "file.txt").read_text(encoding="utf-8") == "working\n"
    assert (target / "new 中文.txt").read_text(encoding="utf-8") == "ordinary untracked\n"
...
def test_kimi_fork_keeps_current_head_and_saved_pin_without_upstream_check(tmp_path, monkeypatch, capsys, dirty, available):
    ...
    if available:
        ...
        selected.write_text(json.dumps(f.receipt_old), encoding="utf-8")
    ...
    selection = json.loads((target / ".vaws-local/environment-selection" / f"{sys.platform}.json").read_text(encoding="utf-8"))
    assert selection == f.receipt_old
    if dirty:
        assert (target / "README").read_text(encoding="utf-8") == "working draft\n"
        assert (target / "new.txt").read_text(encoding="utf-8") == "untracked draft\n"
...
def test_failed_fork_copy_cannot_be_resumed_as_a_ready_worktree(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="preparation is incomplete"):
        adapter.setup(source, source, payload)
    assert (source / "file.txt").read_text(encoding="utf-8") == "concurrent edit\n"
