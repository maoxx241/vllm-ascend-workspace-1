...
AGENTS points to the one-time [repo-init reference](.agents/bootstrap/repo-init/SKILL.md) for first setup. Identity confirmation applies to requested setup or a managed operation that actually needs a missing personal-container identity. Ordinary review and explicit remote I/O do not trigger it. Development forks belong to personal users, with `origin` pointing to the personal fork and `upstream` retaining the official source.
