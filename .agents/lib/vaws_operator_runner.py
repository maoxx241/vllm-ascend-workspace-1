...
MARKER = "__VAWS_OPERATOR_RESULT__"
...
def ensure_operator_entry(root, entry_file):
    """Use the existing Windows owner handoff before reading business files."""
    from vaws_local_owner import windows_mounted_workspace, managed_path
    from vaws_managed_entry import ensure_managed_entry
    if windows_mounted_workspace(root):
        convert = lambda path: managed_path(path, windows=True)
        sys.argv[:] = owner_arguments(sys.argv, convert)
        if hasattr(sys, "orig_argv"):
            sys.orig_argv[:] = owner_arguments(sys.orig_argv, convert)
...
def main(argv=None, *, benchmark=False, workspace_root=None):
    ...
    try:
        ...
        parent = root / ".vaws-local/operator-runs"
