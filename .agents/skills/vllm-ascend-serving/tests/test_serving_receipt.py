...
sys.path[:0] = [str(ROOT / '.agents/skills/vllm-ascend-serving/scripts'),
               str(ROOT / '.agents/lib')]
import _serving_common as common
import _serving_start as start
import vaws_result_envelope as envelope
...
class ReceiptTests(unittest.TestCase):
    def test_compact_and_full_keep_same_business_facts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(common, 'ROOT', Path(tmp)):
            for full in ('0', '1'):
                stream = io.StringIO()
                with mock.patch.dict(os.environ, VAWS_FULL_ENVELOPE=full), mock.patch('sys.stdout', stream):
    ...
    def test_record_failure_preserves_failure_cause_and_exit(self):
        ...
        with mock.patch.dict(os.environ, VAWS_FULL_ENVELOPE='0'), \
